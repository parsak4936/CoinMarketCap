...
export default function IndexTable() {
  ...
  const { symbol, Indexes, loading } = CryptoState();
  const [selected, setSelected] = React.useState([]);
  //   {
  //     "name": "SUSHI",
  //     "id": "USDC",
  //     "market": "SynFutures",
  //     "last": 0.798257919869282,
  ...
  const handleSearch = () => {
    return Indexes.filter((Indexes) =>
      Indexes.name.toLowerCase().includes(search)
    );
  };

  return (
    <ThemeProvider theme={darkTheme}>
      <Container style={{ fontFamily: "irs", textAlign: "center" }}>
        <Typography variant="h4" style={{ margin: 18, fontFamily: "irs " }}>
                ...
                {handleSearch()
                  ...
                  .map((row) => {
                    return (
                      <TableRow
                        // onClick={() => {
                        //   history.push(`/Indexes/${row.market}/${row.id}`);
                        // }}
                      ...
                      >
                        ...
                        <TableCell
                          align="right"
                          style={{ fontFamily: "irl !important" }}
                        >
                          {symbol}{" "}
                          <span
                            style={{
